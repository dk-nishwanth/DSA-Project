...
import { Play, Pause, RotateCcw, GitBranch, ArrowDown, ArrowLeft, ArrowRight } from 'lucide-react';
...
interface TraversalStep {
  ...
  callStack: string[];
  message: string;
  phase: 'at-root' | 'going-left' | 'going-right' | 'complete';
  highlightPath: number[];
}

type TreeType = 'balanced' | 'skewed' | 'complete';

export function PreorderTraversalUniqueViz() {
  ...
  const generatePreorderSteps = (root: TreeNode | null): TraversalStep[] => {
    ...
    const preorder = (node: TreeNode | null, path: number[] = []) => {
      ...
      // Process root FIRST (key difference from inorder)
      visited.push(node.val);
      steps.push({
        currentNode: node.val,
        visited: [...visited],
        callStack: [...callStack],
        message: `Process ROOT first: Add ${node.val} to result`,
        phase: 'at-root',
      ...
      steps.push({
        ...
        visited: [...visited],
        callStack: [...callStack],
        message: `Node ${node.val} - Going LEFT`,
        phase: 'going-left',
        highlightPath: currentPath
      });
    ...
    steps.push({
      ...
      visited: [...visited],
      callStack: [],
      message: `Complete! Preorder result: [${visited.join(', ')}] - Root processed first!`,
      phase: 'complete',
      highlightPath: []
    });
  ...
  const startVisualization = () => {
    ...
    const newSteps = generatePreorderSteps(selectedTree);
    setSteps(newSteps);
    setCurrentStep(0);
    setIsPlaying(false);
  };
  ...
  const renderTree = (node: TreeNode | null, currentStepData: TraversalStep): JSX.Element | null => {
    ...
    const getNodeColor = () => {
      if (isCurrentNode) {
        if (currentStepData.phase === 'at-root') return 'fill-orange-500';
    ...
    <div className="space-y-6">
      <div className="text-center">
        ...
        <p className="text-muted-foreground">
          Process root first, then left subtree, then right subtree - perfect for tree copying!
        </p>
      </div>
      ...
      {steps.length > 0 && currentStepData && (
        <div className="space-y-6">
          <div className={`rounded-lg p-4 text-center border-2 ${
            currentStepData.phase === 'at-root' ? 'bg-orange-100 border-orange-400 text-orange-800' :
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="bg-card border rounded-lg p-4">
              <h4 className="font-semibold mb-3 text-orange-600">Traversal Result</h4>
          ...
          <div className="flex items-center justify-between gap-4">
            ...
            <div className="flex-1">
              ...
              <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
                <div
                  className="bg-gradient-to-r from-orange-600 via-blue-600 to-purple-600 h-2 rounded-full transition-all"
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-card border rounded-lg p-4">
          <h4 className="font-semibold mb-3 text-orange-600">📚 Order</h4>
          <div className="space-y-2 text-sm">
            <div>1. <strong>ROOT</strong> node first</div>
            <div>2. <strong>LEFT</strong> subtree</div>
            <div>3. <strong>RIGHT</strong> subtree</div>
            <div className="pt-2 border-t">
              <strong>Use:</strong> Tree copying, serialization
            </div>
          </div>
        </div>
        ...
        <div className="bg-card border rounded-lg p-4">
          ...
          <div className="space-y-2 text-sm">
            <div>• Create copy of tree</div>
            <div>• Serialize tree structure</div>
            <div>• Prefix expression</div>
            <div>• Tree construction</div>
          </div>
        </div>
      </div>
    </div>
